$(document).ready(function(){
 /*
*buscar un producto para la salida del inventario
*
*/
$('#buscar_producto_salida').on('keyup', function() {
		$.ajax({
...
let ids_para_los_objetos = 0;
function Agregar_Producto_A_La_Salida(codigo, descripcion) {
	//alert(generarCodigo());
  $('#tbl_carrito').show();
  $('#tbl_productos').hide();
  $('#buscar_producto_salida').val('');
  ...
  codigoProducto.id =  `codigoproducto${ids_para_los_objetos}`;
  ...
  txtCantidad.id =  `cantidad${ids_para_los_objetos}`;
  ...
  precioProducto.id =  `precioProducto${ids_para_los_objetos}`;
  ...
  subTotal.id =  `subTotal${ids_para_los_objetos}`;
  ...
  var btnEliminar = document.createElement('button');
	...
	btnEliminar.id =  `btnEliminar${ids_para_los_objetos}`;
    ...
    if (event.keyCode === 13) { // Si se presiona Enter
	 ...
	 // Agrega la última fila con el link de procesar cotización
	  ...
	  linkProcesar.onclick = function(event) {
		  event.preventDefault();//evita que ponga el # en la barra de direcciones
		  //valido el formulario
		 if ($('#frm_agregar_salida_al_inventario').valid()){
		  ...
		  //Código de la función que se ejecuta al hacer clic en el enlace
			$.ajax({
				url: baseUrl + '/Inventario_Salidas/Procesar_Salida',
				type: 'POST',
				data: $('#frm_agregar_salida_al_inventario').serialize(),
				...
				success: function(datos_esperados) {
					...
					if(datos_esperados.respuesta == 1){
						 setTimeout(()=>{
								window.location.replace(baseUrl +'/Inventario_Salidas/Administrar_Salidas_Al_Inventario');
	  ...
	  linkProcesar.textContent = 'Procesar la salida';
  ...
  //para que agregue un id unico para cada elemento:
   ids_para_los_objetos++;
}
/*
*valido el formulario de agregar la entrada al inventario
*/
$('#frm_agregar_salida_al_inventario').validate({
    rules:{
				sa_numero_documento:{required:true}
			},
			messages:{
				sa_numero_documento:{required:'Dato requrido'}
			}
});

function Traer_Detalle_Salida_Inventario(id_salida){
	$.ajax({
			type: 'POST',
			url: baseUrl + '/Inventario_Salidas/Traer_Detalle_Salida_Inventario',
			...
			success: function(data) {
				...
				$('#tbl_salida_elegida').html(data);
				// Muestra una modal para el detalle de la entrada al inventario
				$('#modal_salida_inventario').modal('show');
			},
			complete: function() {
				// Oculta el overlay después de completar la petición
				$('#overlay').hide();
